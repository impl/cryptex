defmodule Cryptex.Hasher.Algorithm.WhirlpoolTest do
  ...
  test "known hashes are correctly calculated" do
    @golden |> Enum.map(fn {known, input} ->
      digest = Whirlpool.new([]) |> Whirlpool.update(input) |> Whirlpool.digest |> Base.encode16
      assert known == digest
    end)
  end
