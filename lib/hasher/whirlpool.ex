# ISO/IEC 10118-3:2004 Whirlpool hashing algorithm.

# Based on Jimmy Zelinskie's Go implementation.
...
defmodule ExCrypto.Hasher.Whirlpool do
  ...
  defstruct [
    bit_length: 1..@length_bytes |> Enum.map(fn _ -> 0 end),
  ...
  defp update_bit_length(bit_length, data_bits) do
    update_bit_length([], bit_length, 0, data_bits)
  end
  ...
  defp update_buffer(%Whirlpool{buffer_bits: buffer_bits} = w, data, data_bits) do
    data_gap = (8 - (data_bits &&& 7)) &&& 7
    buffer_rem = buffer_bits &&& 7

    update_buffer(w, data, data_gap, buffer_rem)
  end
  ...
  defp update_buffer_tail(%Whirlpool{buffer_bits: buffer_bits} = w, _b, bits, buffer_rem) when buffer_rem + bits < 8 do
